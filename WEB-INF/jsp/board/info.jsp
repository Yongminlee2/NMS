...
<head>
	...
	<c:set var="pageName" 		value="${pageContext.request.contextPath}/board/info" />
...
//			$(".info-insert").hide();
	...
	function fnSettingBoard(type){
		...
		if(type=="insert"){
			...
			$("#contents").html("");
			$(".update").show();
			$("#info-btn1").attr("src","<c:url value='/img/btt_note_save.png'/>");
			$("#info-btn1").on("click",function(){
				if(confirm("입력하시겠습니까?")){
					$.ajax({
						url: "${pageName}/insert.ws",
						type: "POST",
				        dataType: 'json',
			...
			$("#info-btn2").attr("src","<c:url value='/img/btt_note_cancle.png'/>");
			$("#info-btn2").on("click",function(){
				if(confirm("취소하시겠습니까?")){
					fnReturnList();
				}
			});				
			...
			$(".update").show();
			$("#info-btn1").attr("src","<c:url value='/img/btt_note_modify.png'/>");
			$("#info-btn1").on("click",function(){
				if(confirm("수정하시겠습니까?")){
					$.ajax({
			...
			$("#info-btn2").attr("src","<c:url value='/img/btt_note_delt.png'/>");
			$("#info-btn2").on("click",function(){
				if(confirm("취소하시겠습니까?")){
					fnSettingBoard("normal");
				}
	...
	<div id="wrap" class="info-list">
	   ...
	   <section class="con_body">
	       ...
	       <article class="s_table2">
             ...
             <p class="f14"> <span onclick="fnCallInsertInfo()"><a href="#" ><img style="width:80px;" src="<c:url value="/img/btt_add.png"/>" alt="추가"></a></span></p>
             <div style="min-height:535px;">
	             <table>
	               ...
	               <c:if test="${totalCnt < 1 }">
	               		<tr>
	               			<td style="height:44px;" colspan="4">검색된 자료가 없습니다.</td>
	               		</tr>
	               </c:if>		               
					<c:forEach items="${infoList}" var="info" varStatus="idx">
						<c:if test="${(idx.index+1) % 2 eq 0}">
							<tr style="height:44px;"onclick="fnDetailBoard('${info.no}')" class="bg_gray2">
						</c:if>
						<c:if test="${(idx.index+1) % 2 ne 0}">
							<tr style="height:44px;" onclick="fnDetailBoard('${info.no}')">
						</c:if>
							<td>${info.no }</td>
							<td>${info.board_title }</td>
							<td>${info.user_id }</td>
							<td>${info.board_dt }</td>
						</tr>
					</c:forEach>
			...
			<tag:paging url="${pageName }/list" firstPage="${pagingInfo.firstPage }" endPage="${pagingInfo.endPage }" lastPage="${pagingInfo.totalLastPage }" curPage="${pagingInfo.currentPage }" totalCnt="${totalCnt }"/>
          </article>
	    </section>
	    
	</div>
	<div id="wrap"  class="info-view" style="display: none;">
	   <!--## subTitle & Address ##-->
	   <section class="con_title">
	     <!--서브제목-->
	     <p class="sTit">자료실</p>
	   ...
	   <section class="con_body">
          ...
          <form id="info-form" action="${pageName }/fileUpload.ws" method="post" enctype="multipart/form-data">
          	<table>
          		<tr><td>
	               <ul>
               ...
               <tr><td>
               <div id="contents" class="bWright" style="width:1437px;min-height: 300px;max-width: 1437px;">abcd deeasd <br> aasd <br> asda <br> asdas </div>
               </td></tr>
<!--                <textarea contenteditable="true" name="message" class="bWright" rows="30"></textarea> -->
               ...
               <ul class="boardBtt">
                 ...
                 <li class="fRight">
                   <p style="padding-left:160px;"><a href="#"><img id="info-btn1" src="<c:url value="/img/btt_note_save.png"/>" alt="목록"></a></p>
                   <p style="padding-left: 30px;"><a href="#"><img id="info-btn2" src="<c:url value="/img/btt_note_cancle.png"/>" alt="목록"></a></p>
                 </li>
               </ul>
               </td></tr>
              </table>
	...
	<div id="wrap"  class="info-insert" style="display: none;">
	   <!--## subTitle & Address ##-->
	   <section class="con_title">
	     <!--서브제목-->
	     <p class="sTit">자료실</p>
